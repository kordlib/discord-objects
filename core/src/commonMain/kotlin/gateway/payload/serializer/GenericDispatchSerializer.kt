...
internal inline fun <reified D : Any, reified T : Dispatch<D>> DispatchSerializer(
    noinline constructor: (D, Int) -> T
): KSerializer<T> = GenericDispatchSerializer(
    T::class,
    ContextualSerializer(
        D::class,
        serializerOrNull(typeOf<D>()) as? KSerializer<D>,
        GenericDispatchSerializer.EMPTY_SERIALIZER_ARRAY
    ),
    constructor
)

internal open class GenericDispatchSerializer<D: Any?, T : Dispatch<D>>(
    klass: KClass<T>,
    private val dataSerializer: KSerializer<D>,
    private val constructor: (D, Int) -> T
) : KSerializer<T> {
    ...
    override val descriptor: SerialDescriptor =
        buildClassSerialDescriptor("dev.kord.discord.objects.payload.${klass.simpleName}") {
            ...
            element("t", EventName.serializer().descriptor, isOptional = false) //event name
            element<Int>("s", isOptional = false) //sequence
        ...
        decoder.decodeStructure(descriptor) {
            while (true) {
                when (val index = this.decodeElementIndex(descriptor)) {
                    ...
                    2 -> seq = decodeIntElement(descriptor, index)
            ...
            val event = constructor(data as D, seq)
            if (eventName != event.name.value) throw SerializationException(
                "EventName ${event.name.value} was expected but got $eventName"
            )
            return event
        }
    }
    ...
    override fun serialize(encoder: Encoder, value: T) {
        encoder.encodeStructure(descriptor) {
            ...
            encodeSerializableElement(descriptor, 1, EventName.serializer(), value.name)
            encodeIntElement(descriptor, 2, value.sequence)
